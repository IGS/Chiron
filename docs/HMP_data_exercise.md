...
mkdir /output/ex1
cd /output/ex1
cp /tutorials/hmp_client/hmp_data/ex1/community_profiles.tsv .
...
```
cp /tutorials/hmp_client/hmp_data_exercise/ex1/16s_metadata.tsv .
cp /tutorials/hmp_client/hmp_data_exercise/ex1/wgs_metadata.tsv .
```
...
#### Create the matrices from MetaPhlAn2 and Qiime matrices.
...
The following command will generate the abundance matrices for the specified subset of samples.
```
Rscript extract_subset.R --metaphlan hmp1-II_metaphlan2-mtd-qcd.pcl.txt \
...
Copy the manifest files <em>stool_rand_wgs_manifest.tsv</em> and <em>stool_rand_16s_manifest.tsv</em> in the examples directory to the local directory and use the script <em>hmp_client</em> to download the data files specified in the manifest file.
