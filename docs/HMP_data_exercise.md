...
1. [Compare 16S and WGS community profiles for a body site](#compare_16s_wgs)
  1. [Download Qiime and MetaPhlAn2 profiles](#download_profiles)
  2. [Create random subset of samples for the body site](#create_random_subsamples)
  3. [Extract matrices for subsamples](#extract_wgs_16s_matrices)
  4. [Run Metaviz to visualize and compare the profiles](#run_metaviz_for_wgs_16s)
2. [Compare the 16S community profiles for two different body sites](#compare_16s_across_sites)
  1. [Create random subset of samples for two body sites](#create_random_subsamples_for_two_sites)
  2. [Extract matrices for subsamples](#extract_two_site_matrices)
  3. [Run Metaviz to visualize and compare the profiles](#run_metaviz_for_two_sites)
...
hmp_client -manifest community_profiles.tsv -destination /output/ex1
```
...
### <a name="create_random_subsamples"></a>Create a list of samples where we have 16S and WGS data
...
#### <a name="extract_wgs_16s_matrices"></a>Extract the matrices from MetaPhlAn2 and Qiime matrices.
...
#### <a name="run_metaviz_for_wgs_16s"></a>Run Metaviz to compare the two abundance matrices
...
#### Extract Qiime subset that will be used for the exercise.
Use the script extract_subset.R to take the list of samples identified in the previous step and the community profiles generated by Qiime and MetaPhlAn2 and extract out the profiles for these matched samples.
...
#### <a name="run_metaviz_for_two_sites"></a>Run Metaviz to compare the abundance matrices for two sites
...
### Download the 16S trimmed sequence sets for the two specified body sites
...
mkdir /output/ex3
cd /output/ex3
cp /tutorials/hmp_client/hmp_data_exercise/stool_rand_wgs_manifest.tsv .
cp /tutorials/hmp_client/hmp_data_exercise/stool_rand_16s_manifest.tsv .
```
...
mkdir /output/ex4
cd /output/ex4
cp /tutorials/hmp_client/hmp_data_exercise/stool_nares_rand_16s_manifest.tsv .
```
...
```
cp examples/hmp_data_exercise/stool_rand_16s_manifest.tsv examples//hmp_data_exercise/stool_rand_wgs_manifest.tsv .
hmp_client -manifest stool_nares_rand_16s_manifest.tsv -destination stool_nares_16s
exit
```
