...
new_mr_have_both <- new_mr_genus_merged[which(!is.na(fData(new_mr_genus_merged)[,"genus_16s"])),]

library(metavizr)
#app <- startMetaviz(host="http://metaviz-dev.cbcb.umd.edu")
#ic <- app$plot(new_mr_have_both, datasource_name = "testing", feature_order = colnames(fData(new_mr_have_both))[1:6])

mobj <- metavizr:::EpivizMetagenomicsData$new(new_mr_have_both, feature_order=colnames(fData(new_mr_have_both))[1:6])
